...
class Reader:
    ...
    def open_excel(self, srcfile):
        # 如果打开的文件不存在，报错；短路原则，如果没有这个文件路径，就直接退出
...
if __name__ == '__main__':
    # reader = Reader()
    # reader.open_excel('../conf/lib/HTTP接口用例.xls')
    ...
    writer = Writer()
    writer.copy_open('../conf/lib/HTTP接口用例.xls', '../results/result-HTTP接口用例.xls')
    sheetname = writer.get_sheets()
    print(sheetname)
    writer.set_sheet(sheetname[0])

    writer.write(1,1,'chf')
    writer.save_close()
